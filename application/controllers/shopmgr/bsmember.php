...
class Bsmember extends CI_Controller {
    ...
    public function login(){
        ...
        $this->load->view('admin/member/loginform') ; 
    }
    ...
    public function join(){ 
        ...
        if($email){
            ...
            $this->load->library('tank_auth');  
            $this->load->view('shopmgr/member/joinform') ; 
        }
    } 
    ...
    public function do_join(){
        ...
        if($data){
            //$arr = explode('@',$this->form_validation->set_value('email'))  ; 
            $args->user_id = $this->form_validation->set_value('email') ; 
            $args->user_name = $this->form_validation->set_value('username') ; 
            $args->nick_name = $args->user_id; 
            $args->email_address = $this->form_validation->set_value('email') ;
            $args->password = $this->form_validation->set_value('password') ; 
    ...
    public function welcome(){ 
        ...
        if($this->users->hasShop($logged_info['bs_user_id'])){
            $shop = $this->users->getShop($logged_info['bs_user_id'])  ; 
            $logged_info['shop'] = $shop ; 
            $this->bsxe->setLoggedInfo($logged_info) ; 

            redirect('shopmgr/statistics/info') ; 
        }else{ 
            $this->load->view('shopmgr/member/shopform') ; 
        }
    ...
    public function registerShop(){
        ...
        $obj->content = 'shopinfo' ; 
        $obj->module_srl = 149 ; 

        $shop_tel = $this->input->post('phone1').'|@|'.$this->input->post('phone2').'|@|'.$this->input->post('phone3') ; 
        $obj->extra_vars1 = getNextSequence()   ; 
        $obj->extra_vars2 = $this->input->post('shop_type')   ; 
        $obj->extra_vars3 = $shop_tel ; 
        $obj->extra_vars4 = $this->input->post('shop_address') ; 
        $obj->extra_vars5 = $this->input->post('shop_time')  ; 
        ...
        $shop_code = $oDocument->getExtraEidValue('shop_id') ; 

        $this->load->model('bettershop/shop_model') ; 

        $args->shop_code = $shop_code; 
        $args->shop_name = $obj->title ; 
