/**
 * @file   modules/bodex/js/bodex_admin.js
 * @author zero (zero@nzeo.com)
 * @brief  bodex 모듈의 관리자용 javascript
 **/


/* 모듈 생성 후 */
function completeInsertBodex(ret_obj) {
    var error = ret_obj['error'];
    var message = ret_obj['message'];

    var page = ret_obj['page'];
    var module_srl = ret_obj['module_srl'];

    alert(message);

    var url = current_url.setQuery('act','dispBodexAdminBoardInfo');
    if(module_srl) url = url.setQuery('module_srl',module_srl);
    if(page) url.setQuery('page',page);
    location.href = url;
}

/* 모듈 삭제 후 */
function completeDeleteBodex(ret_obj) {
    var error = ret_obj['error'];
    var message = ret_obj['message'];
    var page = ret_obj['page'];
    alert(message);

    var url = current_url.setQuery('act','dispBodexAdminBoardList').setQuery('module_srl','');
    if(page) url = url.setQuery('page',page);
    location.href = url;
}

/* 카테고리 관련 작업들 */
function doUpdateCategory(category_srl, mode, message) {
    if(typeof(message)!='undefined'&&!confirm(message)) return;

    var fo_obj = xGetElementById('fo_category_info');
    fo_obj.category_srl.value = category_srl;
    fo_obj.mode.value = mode;

    procFilter(fo_obj, update_category);
}

/* 카테고리 정보 수정 후 */
function completeUpdateCategory(ret_obj) {
    var error = ret_obj['error'];
    var message = ret_obj['message'];
    var module_srl = ret_obj['module_srl'];
    var page = ret_obj['page'];
    alert(message);

    var url = current_url.setQuery('module_srl',module_srl).setQuery('act','dispBodexAdminCategoryInfo');
    if(page) url.setQuery('page',page);
    location.href = url;
}

/* 일괄 설정 */
function doCartSetup(url) {
    var module_srl = new Array();
    jQuery('#fo_list input[name=cart]:checked').each(function() {
        module_srl[module_srl.length] = jQuery(this).val();
    });

    if(module_srl.length<1) return;

    url += "&module_srls="+module_srl.join(',');
    popopen(url,'modulesSetup');
}

/* 목록 설정 */
function doInsertItem(obj) {
    if(obj=='sort')
    {
        var target_obj = xGetElementById('displayItem');
        var display_obj = xGetElementById('sortItem');
    }else{
        var target_obj = xGetElementById('targetItem');
        var display_obj = xGetElementById('displayItem');
    }
    if(!target_obj || !display_obj) return;

    var text = target_obj.options[target_obj.selectedIndex].text;
    var value = target_obj.options[target_obj.selectedIndex].value;

    for(var i=0;i<display_obj.options.length;i++) if(display_obj.options[i].value == value) return;

    var obj = new Option(text, value, true, true);
    display_obj.options[display_obj.options.length] = obj;

}
function doDeleteItem(obj) {
    if(obj=='sort') var sel_obj = xGetElementById('sortItem');
    else var sel_obj = xGetElementById('displayItem');
    var idx = sel_obj.selectedIndex;

    if(obj!='sort' && (idx<0 || sel_obj.options.length<2)) return;

    var value = sel_obj.options[idx].value;

    sel_obj.remove(idx);
    sel_obj.selectedIndex = idx-1;

    /* phiDel (2010.01.19) 정렬 아이템도 같이 제거  */
    if(obj!='sort'){
        var sort_obj = xGetElementById('sortItem');
        for(var i=0;i<sort_obj.options.length;i++)
            if(sort_obj.options[i].value == value){
                sort_obj.remove(i);
                sort_obj.selectedIndex = i-1;
            };
    }
}
function doMoveUpItem() {
    var sel_obj = xGetElementById('displayItem');
    var idx = sel_obj.selectedIndex;
    if(idx<1 || !idx) return;

    var text = sel_obj.options[idx].text;
    var value = sel_obj.options[idx].value;

    sel_obj.options[idx].text = sel_obj.options[idx-1].text;
    sel_obj.options[idx].value = sel_obj.options[idx-1].value;
    sel_obj.options[idx-1].text = text;
    sel_obj.options[idx-1].value = value;
    sel_obj.selectedIndex = idx-1;
}
function doMoveDownItem() {
    var sel_obj = xGetElementById('displayItem');
    var idx = sel_obj.selectedIndex;
    if(idx>=sel_obj.options.length-1) return;

    var text = sel_obj.options[idx].text;
    var value = sel_obj.options[idx].value;

    sel_obj.options[idx].text = sel_obj.options[idx+1].text;
    sel_obj.options[idx].value = sel_obj.options[idx+1].value;
    sel_obj.options[idx+1].text = text;
    sel_obj.options[idx+1].value = value;
    sel_obj.selectedIndex = idx+1;
}

function doSaveListConfig(module_srl) {
    if(!module_srl) return;
    var sel_obj = xGetElementById('displayItem');

    var idx = sel_obj.selectedIndex;

    var list = new Array();
    for(var i=0;i<sel_obj.options.length;i++) list[list.length] = sel_obj.options[i].value;
    if(list.length<1) return;

    /* phiDel (2010.01.19) 정렬 아이템 목록  */
    var sor_obj = xGetElementById('sortItem');
    var sort_list = new Array();
    for(var i=0;i<sor_obj.options.length;i++) sort_list[sort_list.length] = sor_obj.options[i].value;

    var params = new Array();
    params['module_srl'] = module_srl;
    params['list'] = list.join(',');
    params['sort_list'] = sort_list.join(',');

    var response_tags = new Array('error','message');

    exec_xml('bodex','procBodexAdminInsertListConfig', params, function() { location.reload(); });
}

function doSaveSearchConfig(module_srl) {
    if(!module_srl) return;
    var sel_obj = xGetElementById('displayItem');

    var idx = sel_obj.selectedIndex;

    var list = new Array();
    for(var i=0;i<sel_obj.options.length;i++) list[list.length] = sel_obj.options[i].value;
    if(list.length<1) return;

    var params = new Array();
    params['module_srl'] = module_srl;
    params['list'] = list.join(',');

    var response_tags = new Array('error','message');

    exec_xml('bodex','procBodexAdminInsertSearchConfig', params, function() { location.reload(); });
}

/* 추천필드갱신 */
function doRecountVoted(module_srl, use_vote) {
    if(!module_srl) return;

    if(!confirm('* Warning *\n\nThis may take some time depending on your data size.\nWould you like to continue?')) return;

    var params = new Array();
    params['module_srl'] = module_srl;
    params['use_vote'] = use_vote;

    var response_tags = new Array('error','message');

    exec_xml('bodex','procBodexAdminRecountVoted', params);
}
