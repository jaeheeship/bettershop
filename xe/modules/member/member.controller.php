<?php
    ...
    class memberController extends member {
        ...
        /**
         * @brief 회원 가입 ( iOS 모바일 회원가입 by j.kim)
         **/
        function procMemberInsertApp() {
		//return 'AppRegister module by j.kim';
			$oMemberModel = &getModel ('member');
			$config = $oMemberModel->getMemberConfig ();   
			
			// 관리자가 회원가입을 허락하였는지 검사
			if ($config->enable_join != 'Y') return '현재 회원을 더 받고 있지 않음';
		...
		/*
			$args->user_id="kjy9999";
			$args->user_name="김진용";
			$args->nick_name="진용이";
			$args->homepage="";
			$args->blog="";
			$args->birthday="";
			$args->email_address="j.kim22@xiilab.com";
			$args->password="";
			$args->allow_mailing="N";
			$args->find_account_question="";
		...
		*/
			...
			if($config->redirect_url) $this->add('redirect_url', 'http://market.android.com');
			//if ($config->enable_confirm == 'Y') {
			//	$msg = sprintf(Context::getLang('msg_confirm_mail_sent'), $args->email_address);
			//	$this->setMessage($msg);
			//}
