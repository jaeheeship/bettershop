function insert_filelink(form){ return legacy_filter('insert_filelink', form, 'bodex', 'procBoardInsertFileLink', completeInsertFileLink, ['error','message','editor_sequence_srl'], '', {}) };
(function($){
	var v=xe.getApp('validator')[0];if(!v)return false;
	v.cast("ADD_FILTER", ["insert_filelink", {'filelink_url': {required:true,minlength:5}}]);
	
	v.cast('ADD_MESSAGE',['filelink_url','파일 주소']);
	v.cast('ADD_MESSAGE',['mid','모듈 이름']);
	v.cast('ADD_MESSAGE',['document_srl','문서번호']);
	v.cast('ADD_MESSAGE',['editor_sequence_srl','editor_sequence_srl']);
