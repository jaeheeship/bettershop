function toggle_activate_addon(form){ return legacy_filter('toggle_activate_addon', form, 'addon', 'procAddonAdminToggleActivate', reloadDocument, ['error','message'], '', {}) };
(function($){
	var v=xe.getApp('validator')[0];if(!v)return false;
	v.cast("ADD_FILTER", ["toggle_activate_addon", {'addon': {required:true},'type': {required:true}}]);
	
	v.cast('ADD_MESSAGE',['addon','애드온']);
	v.cast('ADD_MESSAGE',['type','type']);
